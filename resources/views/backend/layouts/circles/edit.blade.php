@extends('backend.app', ['title' => 'Edit Circle'])
...
<div class="app-content main-content mt-0">
    <div class="side-app">
        <div class="main-container container-fluid">
            ...
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.circles.update', $circle->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="video_id" id="video_id" class="form-control @error('video_id') is-invalid @enderror">
                                ...
                                @foreach($videos as $video)
                                    <option value="{{ $video->id }}" {{ old('video_id', $circle->video_id) == $video->id ? 'selected' : '' }}>
                                        {{ $video->title }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" name="title" class="form-control" value="{{ old('title', $circle->title) }}">
                        </div>

                        <!-- Description -->
                        <div class="form-group mb-3">
                            <label for="description">Description:</label>
                            <textarea name="description" class="form-control">{{ old('description', $circle->description) }}</textarea>
                        </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="file" name="image" class="dropify form-control"
                                data-default-file="{{ $circle->image ? asset($circle->image) : '' }}">
